...
function AliOssService(options) {
    ...
    if ('function' === typeof options.getFilename) {
        self.getFilename = options.getFilename;
    }
    ...
    self.oss = new ALY.OSS({
        ...
        endpoint: self.options.protocol + '://' + (self.options.internal ? 'vpc100-' : '') + self.options.region + '.aliyuncs.com',
...
AliOssService.prototype.upload = function (req, res, options, cb) {
    ...
    busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
        ...
        if (self.getFilename) {
            var fileInfo = { name: filename };
            key = self.getFilename(fileInfo, req, res);
        }

        var upload = self.ossStream.upload({
            Bucket: self.options.bucket || req.params.container,
            Key: key,
            ContentType: mimetype,
            CacheControl: 'max-age=31536000',
            Expires: new Date(Date.now() + 94608000 * 1000)
        });

        upload.on('uploaded', function (details) {
          details['originalFilename'] = filename;
          cb(null, details);
        });
...
AliOssService.prototype.download = function (container, file, req, res, cb) {
    ...
    self.oss.getObject({
        ...
        if (err || !content) {
            ...
            res.status((err && err.statusCode) || 500).send(errorHandler(err));
        } else {
